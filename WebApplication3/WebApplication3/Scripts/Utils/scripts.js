...
$("#logradouro").change(function () {
    ...
    var newdata = FindByLog($('#logradouro').val());

    $('#bairro').val(newdata.bairro);
    $('#localidade').val(newdata.localidade);
    $('#uf').val(newdata.uf);
});
...
function FindByLog(logradouro) {
    ...
    ////$.getJSON("https://viacep.com.br/ws/RS/Porto%20Alegre/all/json/", function (data) { //conectando com api web
    //    $.each(data, function (key, value) {
    //        var templateContent = "";
    //        templateContent += "}"; // separador de informacao
    //        $.each(value, function (key, value) {//laco para repetir cada informacao
    //            //adicionamos todas as informacoes do nosso objeto
    //            templateContent += key + ":" + value + ",";
    //        });
    ...
    //        $("localidade").val(value.localidade);
    //        $("uf").val(value.uf);
    //    });
    //});
